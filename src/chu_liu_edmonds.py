...
#@title class Chu-liu/Edmonds - MTriet
class ChuLiuEdmonds:
    ...
    def expand(self, contracted: Arborescence, cycleNode: int, *args, **kwargs) -> Arborescence:
        ...
        for e in contracted.edges:
            if e.v == cycle_id:
                ...
                result_edges.append(getattr(e, 'real_edge', e))
        ...
        for v in cycle_vertices:
            ...
            result_edges.append(cycle_edge)

        if entering is not None:
            result_edges.append(getattr(entering, 'real_edge', entering))

        if len(result_edges) != numV - 1:
            raise ValueError(f"Internal error: expanded edge count mismatch. Expected {numV - 1}, got {len(result_edges)}")
