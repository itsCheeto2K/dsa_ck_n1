...
class ChuLiuEdmonds:
    ...
    def findMinInEdges(self, g: DirectedGraph, root: int):
        """private int[] findMinInEdges(DirectedGraph g, int root)"""
        min_parent, _, _ = self._compute_min_in(g, root)
        return min_parent

    def detectCycle(self, minIn, numV: int) -> int:
        """private int detectCycle(int[] minIn, int numV)"""
        numV = int(numV)
        visited = [0] * numV
        stamp = [0] * numV
    ...
    def contract(self, g: DirectedGraph, cycleNode: int, id):
        """private DirectedGraph contract(DirectedGraph g, int cycleNode, int[] id)"""
    ...
    def expand(self, contracted: Arborescence, cycleNode: int, *args, **kwargs) -> Arborescence:
        """private Arborescence expand(Arborescence contracted, int cycleNode, ...)"""
    ...
    def _compute_min_in(self, g: DirectedGraph, root: int):
        ...
        for v in range(numV):
            ...
            if min_edge_idx[v] == -1:
                raise ValueError(
                    f"No arborescence exists: vertex {v} has no incoming edge"
                )
